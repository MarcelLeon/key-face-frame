...
echo "╔══════════════════════════════════════════════════════════╗"
echo "║                                                          ║"
echo "║          Key-Face-Frame v2.0 - 一键启动脚本              ║"
echo "║          AI视频关键帧提取工具                             ║"
echo "║                                                          ║"
echo "╚══════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
MODEL_CACHE="$HOME/.cache/ultralytics"
if [ ! -f "$MODEL_CACHE/yolov8m.pt" ]; then
    log_warn "YOLOv8模型未下载，首次运行会自动下载（约50MB）"
    log_info "下载位置: $MODEL_CACHE"
    log_info "这可能需要几分钟，请耐心等待..."
else
    log_success "YOLOv8模型已存在"
fi
...
# 检测操作系统，Mac使用--pool=solo
if [[ "$OSTYPE" == "darwin"* ]]; then
    CELERY_POOL="--pool=solo"
    log_info "检测到macOS系统，使用 --pool=solo 模式（Apple Silicon优化）"
else
    CELERY_POOL=""
    log_info "使用默认pool模式"
fi

nohup celery -A backend.workers.tasks worker --loglevel=info $CELERY_POOL > logs/celery.log 2>&1 &
CELERY_PID=$!
