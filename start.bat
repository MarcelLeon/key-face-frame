...
REM 检查YOLOv8模型
REM ==============================================================================
echo %BLUE%[INFO]%NC% 检查YOLOv8模型...

set MODEL_CACHE=%USERPROFILE%\.cache\ultralytics
if not exist "%MODEL_CACHE%\yolov8m.pt" (
    echo %YELLOW%[WARN]%NC% YOLOv8模型未下载，首次运行会自动下载（约50MB）
    echo %BLUE%[INFO]%NC% 下载位置: %MODEL_CACHE%
    echo %BLUE%[INFO]%NC% 这可能需要几分钟，请耐心等待...
) else (
    echo %GREEN%[SUCCESS]%NC% YOLOv8模型已存在
)
